...
class VanguardClient extends SoapHelper
{
    ...
    public function __construct($atoUrl, $usiUrl, $orgCode, $usi, $first, $family, $dob)
    {
        parent::__construct();
        $this->atoUrl = $atoUrl;
        $this->usiUrl = $usiUrl;
    ...
    public function verifyUSI($tokenResponse)
    {
        try {
...
//        $this->doc->save('request-usi.xml');
    ...
    public function loadAuskey($path, $password = '')
    {
        if (!file_exists($path)) {
        ...
        try {
            $this->setAuskey($xml, $password);
        } catch (Exception $ex) {
            throw $ex;
        }
    }

    public function setAuskey($xml, $password = '')
    {
        try {
            $this->auskey = new AUSKey($xml, $password);
        } catch (Exception $ex) {
            throw $ex;
        }

        $this->auskey->setCredential(0);
    }
...
//        $signedinfo = $this->buildSignedInfoSha1();
    ...
    private function getBinarySecurityToken()
    {
        ...
        $pkcs7_file = '/tmp/pkcs7';
        $f = fopen($pkcs7_file, 'w');
        fwrite($f, $this->getPublicCertificatePEM());
        fclose($f);

        // Then we extract the certificates using 'openssl' utility.
        $keys = shell_exec('openssl pkcs7 -print_certs -in ' . $pkcs7_file);
        unlink($pkcs7_file); // We don't want this hanging around in the file system!!!
        if (empty($keys)) {
            return null;
        }
        ...
        $matches = null;
        preg_match_all('/-----BEGIN CERTIFICATE-----(.*)-----END CERTIFICATE-----/mUs', $keys, $matches);
        if (empty($matches)) {
            return null;
        }
    ...
    public function getPublicCertificatePEM()
    {
        $key = "-----BEGIN PKCS7-----\n";
        $key .= chunk_split($this->getAuskey()->getCredential()->publicCertificate, 64, "\n");
        $key .= "-----END PKCS7-----";
    ...
    public function getPrivateKeyPEM()
    {
        ...
        $key .= chunk_split($this->getAuskey()->getCredential()->protectedPrivateKey, 64, "\n");
